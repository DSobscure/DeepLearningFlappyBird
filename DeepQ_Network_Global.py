...
def TrainNetwork(inputStates, outputs, network, interactiveSession):
    ...
    while True:
        ...
        if terminal:
            ...
            globalView = (currentScore - Numpy.mean(numpyScores)) / (Numpy.std(numpyScores) + 0.001);
            print("Game", gameCount, " score: ", currentScore, "GV: ", globalView, " t: ", t);
            with open('result.txt', 'a') as file:
                file.writelines(str(currentScore) + "\n")
            for i in range(0, len(gameRecord)):
                x = gameRecord.popleft()
                sameRecord = (x[0], x[1], currentScore, x[3], x[4])
        ...
        if t > OBSERVE:
            ...
            for i in range(0, len(batch)):
                ...
                if terminal:
                    ...
                    valueBatch.append((rewardBatch[i] - Numpy.mean(numpyScores)) / (Numpy.std(numpyScores) + 0.001) + GAMMA * Numpy.max(afterStateBatchOutput[i]))
